...
import { listProviders, getConfigVersion } from "./config.js";
...
async function getAggregatedModels(env) {
  ...
  for (const p of providers) {
    ...
    for (const m of p.models || []) {
      if (!seen.has(m)) {
        seen.add(m);
        models.push({ id: m, object: "model", created: 0, owned_by: mapTypeToOwner(p.type) });
      }
    }
...
function checkClientAuth(request, env) {
  // If OPENAI_API_KEY is not set, allow all requests (no client auth)
